...
weather <-
  read.csv("2020.csv.gz", header = FALSE)                 # YTD Weather by coop stations
weather <- weather[,-c(5:8)]
colnames(weather) = c("ID", "DATE", "ELEMENT", "VALUE")
weather <-
  filter(
    weather,
    ELEMENT == "TMAX" |                                  # Max Temp  - tenth of degree C
      ELEMENT == "TMIN" |                                # Min Temp   - tenth of degree C
      ELEMENT == "PRCP" |                                # Precipitation - tenth of mm
      ELEMENT == "PSUN" |                                # Percent of sunshine form sunrise until sunset
      ELEMENT == "ACSC" |                                # Cloud cover midnight to midnight - ceilometer measured every 30 seconds
      ELEMENT == "ACSH"                                  # Clooud cover midnight to midnight manual observations
  )                                                      # Unpack weather, filter out unused events
...
county_data %>%                                                   # Widen data by putting TMAX, TMIN, and PRCP in each entry
  ...
  summarise(
    ...
    PSUN = mean(PSUN, na.omit = TRUE),
    ACSC = mean(ACSC, na.omit = TRUE),
    ACSH = mean(ACSH, na.omit = TRUE),
    
  ) -> RoysData

RoysData$FIPS <- sprintf("%05d",RoysData$FIPS)  # County FIPS must be 5 digit so make char with leading zero

saveRDS(RoysData, "roysdata.RDS")               # Not sure how you want the data.  This is a tibble - could make time series
