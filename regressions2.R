...
whiskers <-
  ggarrange(
    ...
    colnames(tab2) <- cn2
    rownames(tab2) <- rn2
    info <- arrangeGrob(tableGrob(format(tab1, digits = 4)),tableGrob(format(tab2, digits = 4)),ncol = 2)
    
    print(annotate_figure(
      # Push page to PDF with Country Name
     # ggarrange(regressions, whiskers, info, nrow = 2),
